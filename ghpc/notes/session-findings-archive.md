# Session findings archive

Chronological findings from bring-up sessions, moved verbatim out of
`BACKLOG.md` on 2026-09-09 because they had grown to roughly 1570 lines and
buried the queue they sat under.

Nothing here was edited. It is append-ordered, not topic-ordered, and later
entries supersede earlier ones. Where a topic has its own note in this
directory (`song-load-crash.md`, `ui-draw-hang.md`, `m5-progress.md`), that
note is the one to trust.
...
## drawseq reports animation as divergence, and could not be aimed

Found while trying to reach the flicker case. Two defects in the tool itself,
both of which would have made any conclusion drawn from it wrong.

**It fired on an animated menu.** The trigger was `calls > 1000000` on
DrawPrimitive with a one-shot `done` latch, so it went off at a fixed draw
count wherever the game happened to be. That is a menu, and a menu animates, so
two consecutive frames legitimately differ. The first report looked like a
result and was not:

    first diverging draw 498 from mscal 13592 (top=31) vs 13599 (top=31)
    vu1 data: 1023 quadwords identical, 1 differ (of 1024)
    +679 A c3163f98 c03fffff c1c3ad50 44400000
         B c3164ce8 c03fffff c1c10d20 44400000

Those decode to -150.24 against -150.30 and -24.46 against -24.13. Sub pixel
deltas, an object moving. Three things say it is not the flicker: both sides at
the same TOP, so the double buffer parity mechanism is not in play at all; 1023
of 1024 quadwords identical; and no vertices on raw (2048, 2048), which is the
signature the real corruption leaves. The divergence test is
`fabs(dx) > 0.01f`, which any animation clears.

**Fixed, uncommitted.** `GHPC_DRAWSEQ` now takes a draw call count so it can be
aimed past the menus, and `GHPC_DRAWSEQ_EVERY` re-arms it so one run yields a
report per interval. The re-arm is a scope exit guard, because the report reads
`n[]` and `seq[][]` after the latch and resetting them inline would silently
gut it.

## The auto pad does not walk into a song, so the flicker was never reached

30 minute run, 28 drawseq reports, pad auto on. Every single report:

    top=31  vs top=31    (18 of 28)
    top=361 vs top=361   (10 of 28)
    first diverging draw 498 in 26 of 28
    frameA n=846 | frameB n=846, every report, fbp alternating 0 and 56

Not one report shows the two sides at different TOPs. Same 846 draw screen for
half an hour. The alternating CROSS and START walk enters a menu and then backs
straight out of it, so it never reaches a song.

`readPadPortData` is definitely being called, so the game does read the pad.
The old `s_logged < 12` cap is why the log goes quiet after 12 presses, which
cannot distinguish "still pressing" from "stopped". Both changed, uncommitted:
CROSS on 7 of every 8 cycles, 1.0s period with a 0.35s press, and rate limited
logging rather than a cap.

## Read against clobber ordering: built, aimed at nothing yet

The measurement the previous pass named as next is implemented and unused,
because the flicker case above was never entered. Uncommitted.

`ghpcLogQwWrite` already recorded stores at **commit** time inside the
`m_storePipeline` drain, which is the correct point in memory order and the
part that would have been easy to get wrong. Missing were reads. Added:

  - read events at LQ, LQI and LQD in `ps2_vu1_lower.cpp`, tag `2 + (pc<<4)`
    alongside the existing `1 + (pc<<4)` for stores and 0 for unpack
  - `GHPC_RW_FROM` and `GHPC_RW_SPAN` replacing the hardcoded qw 49 and 192
    filter, so a window covers the whole 290 quadword input
  - a monotonic `seq` on every event, with the mscal stamped **at event time**
    rather than assigned from a ring index later, which is what caused the off
    by one that produced a confident wrong answer last pass
  - `[gs/rworder]` verdict comparing firstStore against firstRead per quadword,
    plus a full event dump for one offending quadword so the trace can be
    checked before the count is believed

Reads deliberately do not touch `g_ghpcLastWriter`, so the existing last writer
report keeps meaning what it meant.

Verdict semantics, once it can be aimed:

    unpack -> read -> store    safe, program read real input
    unpack -> store -> read    clobbered before read, mechanism CONFIRMED

Span 40 is not arbitrary. At roughly 600 to 1000 accesses per mscal, 40 mscals
is about 40k events against a 65536 entry log. A wider window rolls over and
drops the very mscal it was aimed at, which is the count cap trap wearing a
different hat.

### Cross heavy input changes nothing, so input is not the blocker

Second 30 minute run, CROSS on 7 of 8 cycles, 1760 cycles, roughly 1540 CROSS
presses actually delivered (`buttons=0xbfff` at cycle 1760 is bit 14 cleared,
CROSS active low, so the synthesis is correct all the way through).

    distinct frame draw counts seen:  29 x 846, nothing else
    reports with differing tops:      none
    first diverging draw:             498, throughout

One screen for half an hour, unchanged, across two different input patterns.
Combined with the backtrace evidence from the same state, the picture is:

  - the EE thread runs a healthy per frame UI draw
    (`Run__3App` / `Draw__9UIManager` / `Draw__8GHScreen` / `Draw__8UIScreen` /
    `Draw__7UIPanel` / `DrawShowing__8PanelDir` / `FlushPacket__5PsRnd` /
    `Send__9DmaPacket`), traversing different parts of the scene graph each
    sample, so it is live and not looping on one call
  - the main thread sits in `EndDrawing` and `usleep`, the raylib frame limiter
    behaving normally, and the 111 percent CPU is just the rasterizer
  - no ARK reads at all, across 18 samples and 100 rapid `lsof` polls
  - 1540 CROSS presses reach `fillPadStatus` and change nothing

So the game parks on a screen, renders it forever, reads no data, and ignores
input. **The flicker work is blocked behind this**, not the other way around:
the flicker cannot be measured without reaching the screen it happens on, and
nothing automated gets there.

Noah does advance it by hand, so real input works where synthesized input does
not. That difference is the lead, and it is testable without guessing at button
patterns: find out whether the game actually consumes what `readPadPortData`
produces, or whether the value it acts on arrives by another path. The previous
pass already named this as the missing tooling ("logging of what the game
actually reads"). Do that before touching the pad synthesis again.

Do not try a third button pattern. Two patterns, an hour of runtime, and a
confirmed correct button encoding is enough to say the pattern is not the
variable.

## Two more leads killed, and a systemic problem with the diagnostics

Both measured while stuck on the never finishing load, with pad input running,
two readings about 110 seconds apart so frequency was visible rather than a
single snapshot.

**The unhandled audio RPC is not the hang.** `sid=0x75433178` is the only
unhandled service in any run, and it is almost certainly Harmonix's `synth_r`.
It explains having no audio. It does not explain the hang: the call count sat
at 4 at 110s and was still 4 at 220s. The game asks at startup and never asks
again. Not a poll.

**The null virtual dispatch is not the hang either.** `0x126508` is
`UpdateSongPos__9BeatMatchRC7SongPos +0x468`, a GCC 2.x vtable dispatch:

    1264ec  lw $3, 0xbe8($2)   ; global at 0x440be8
    1264f8  lw $8, 0x0($3)     ; object
    1264fc  lw $2, 0x0($8)     ; object's vtable pointer
    126500  lh $5, 0x30($2)    ; this adjust
    126504  lw $3, 0x34($2)    ; function pointer
    126508  jalr $3            ; resolves to 0

The object's vtable pointer points at memory holding `down` `beat` `\0on_`
`down`, so a live object's storage is being read as a vtable and contains DTA
symbol text instead. That still looks like a real defect and it is in the right
subsystem, but it fires fewer than 200 times in 210 seconds, so it is not what
is spinning. Worth fixing on its own terms, not as the hang.

### The systemic problem

Three separate diagnostics suppress repeats and report only the first
occurrence, which makes "how often does this happen" unanswerable:

  - `logUnhandledRpcTrace` dedupes on (sid, rpcNum, sendSize, recvSize)
  - `m_missingFunctionReported` is ONE global latch for the entire run, across
    every site, so the first missing target anywhere silences all of them
  - `ghpcLogQwWrite` was hardcoded to two quadwords with no window

Frequency was the deciding question three times today and twice the suppressed
count was initially read as if it were a real one. Census counters were added
for the first two and should stay. Prefer a rate limited counter over a first
only line in anything added from here.

### Where the load actually stands

Not I/O bound: zero ARK reads across 18 samples and 100 rapid lsof polls. Not
deadlocked: the EE thread runs a healthy per frame UI draw and the main thread
sits in the raylib frame limiter. Not input starved in any way we can reach:
about 1540 CROSS presses across two patterns changed nothing, and every drawseq
capture in every run is the same 846 draw screen.

Next approach should come from the other end. Stop mining the existing log
lines for candidates. Sample the guest repeatedly while stuck and collect the
distinct call paths outside the draw tree, to find the update or state machine
that is not advancing and what condition it is testing.

## Un-stubbing the allocator family: attempted, reverted, and what it proved

Tried and backed out the same day. Reverted because it regressed the tree: the
game died about 30 seconds in where before it ran indefinitely. Everything below
is worth keeping, because the attempt answered questions that cost a full
recompile each.

### It does work mechanically

Denylisting the eight symbols (malloc, free, realloc, memalign, _malloc_r,
_free_r, _realloc_r, _calloc_r) makes the recompiler emit the real bodies:

    malloc_0x35c778      2 lines of stub  ->  131 lines
    free_0x35c7c8        2 lines          ->  125 lines
    _malloc_r_0x3621b0   2 lines          -> 2347 lines
    memalign_0x35c718    2 lines          ->  143 lines

And the game's allocations stop reaching the runtime arena entirely: the pool
probe's 43 REFUSED lines went to **zero** guestMalloc calls. The flex crash
stayed gone without the reserve doing any work. So the substitution can be
removed cleanly, and the region separation below is sound.

Note the build trap: `--from=recomp` SKIPS the `tools` stage, so
`build/ps2xAnalyzer/ps2_analyzer` does not exist and the recompile silently does
nothing while `build.sh` still reports success (the pipe to `tail` swallows the
status). Use `--from=tools`. Check that the generated files actually changed
rather than trusting the exit code.

### Region separation, which was correct and is worth re-using

Runtime arena moved to a fixed block so it cannot overlap the guest heap:

    game heap   [game base ...... 0x01D00000)   real sbrk, ceiling = EndOfHeap
    runtime     [0x01D00000 ..... 0x01F00000)   guestMalloc only
    stack/async [0x01F00000 ..... 0x02000000)   unchanged

`SetupHeap` must stop calling `configureGuestHeap`, since pointing the runtime
arena at the game's region is the overlap being removed, and must return the
game's own base. `EndOfHeap` returns the arena base as the guest's ceiling.

**Do not use `guestHeapBase()` for that.** It returns the ELF derived
`m_guestHeapSuggestedBase` until the arena is lazily configured, and EndOfHeap
asks during boot, so the game gets a 4KB heap (`ceiling=0x005bf2b0` against
`base=0x005be2b0`) and its allocator dies much later somewhere unrelated. Add an
accessor returning the constant directly. That bug was found and fixed, and the
fix was verified: `ceiling=0x01d00000`.

### The actual blocker, which is upstream of all of the above

With a correct 24MB ceiling, the game still dies identically:

    [guest-branch:missing-target] JALR source=0x3243b8 target=0x14
      a0=0x80 a1=0x0 a2=0x14 a3=0xffffffe0 s0=0x0 v0=0x14
    Error during program execution: EE invocation stack space exhausted

`0x3243b8` is `Refill__14FixedSizeAlloc +0x68`. It trips the game's own
`Debug::Fail` and then dispatches through `*(this+0x18)`, its Heap pointer,
which is null, so the GCC 2.x vtable read (`lh $4,0x10($3)` / `lw $2,0x14($3)`)
pulls the pfn from address 0x14 and gets 0x14.

**`EndOfHeap` is called zero times in that run.** The heap never had to grow, so
the game's Heap object was never constructed in the first place. Un-stubbing
malloc is necessary and not sufficient: something else in the memory manager
init path is still stubbed or wrong, and it runs before any allocation. Find
what constructs the Heap that `FixedSizeAlloc` refills from, and why it is null,
BEFORE attempting the denylist again. Doing them in the other order costs a full
recompile per iteration for no reason.

## Song load never finishes (open, 2026-09-09)

The heap crash is fixed (128MB, commit 6babe38). The game now boots, walks the
menus and enters `loading_screen`, and stays there. 24 minutes on a release
build with no diagnostics, no fatal, no `game_screen`.

Ruled out, with evidence, do not re-chase:

- **Not a semaphore deadlock.** Thread 2 is the async file worker. `curCall ==
  freeCall` with `curFunc=0` is an idle worker on an empty ring, not a block.
- **Not the ARK read path.** 49 distinct ArkFiles reached `pending=0` in one
  run; one read 3055360 bytes to completion. `BlockMgr::Poll` does retire tasks.
- **Not diagnostic overhead.** Release is ~2x faster to boot (t=37 vs t=74) and
  still does not finish the load.
- **Not the SPU stall.** `GHPC_SYNTH_ACK=1` clears the handshake
  (`pending=0 inflight=0`) and the loader queue is byte identical.

Unresolved: the load does real work continuously (`addTask` climbed 216 -> 398
during one load screen) but never completes. Suggestive and unchased: the same
ArkFiles get reopened repeatedly (0x019efd90 went 47->0, then 1->0, then 2->0)
and `LoadMgr::GetLoader` was entered 3000+ times for the same loader. Could be
a re-load cycle. Could be a genuinely huge amount of work. Not yet separated.

**Do this before any more long runs.** Define the pass/fail metric first, then
run. The technique already proven in `notes/song-load-crash.md` is two call
histogram samples across the phase: a loop repeats its delta vector, progress
does not. Needs `build.sh --calls`. A 25 minute run with no success criterion
defined up front tells you nothing, which is how three wrong theories got built
this session.

## Twelve stale 25-bit address masks (2026-09-09)

`0x01FFFFFF` was correct while RAM was 32MB. `PS2_RAM_SIZE` is now 128MB, so
every one of these truncates addresses above 32MB. Fixed one in the heap census
(commit 350741d); these remain, in `EeScheduler.cpp` (7), `ghpc_drive.h` (2) and
the `Debug::Fail` probe in `ps2_runtime.cpp` (2):

They only clip log output, not state, BUT the EE stall census reads thread
stacks and stacks now live at 0x07F8xxxx, so those dumps currently read the
wrong memory and will lie. Strip kseg bits with `0x1FFFFFFF` and bounds check
against `PS2_RAM_SIZE` instead.

## Correct the gCurHeap claim in stub-denylist.txt (2026-09-09)

Line 32 states `MemAlloc__FiPCci` falls back to plain `malloc` whenever
`gCurHeap` is null, "which is always". A linear heap walk at song load found
225 live chunks total across a full boot, with two of them holding 94% of
23.3MB. Thousands of small newlib chunks would be expected if every MemAlloc
fell through. The game is serving most allocations from its own pools. That
note is stale or only true on some paths, and it shaped how this branch was
reasoned about.

## Is the render path actually dead? (2026-09-09)

`CLAUDE.md` says the VIF1 -> VU1 -> GIF path "delivers nothing" into the
framebuffer. Runs this session show frames at 189807/229376 non-black pixels
with 171804 distinct colours, and the pad harness drives off live UI state, so
UI rendering demonstrably works. Either the note is stale or it means 3D world
drawing specifically. Worth settling, because it changes what the next
milestone after "song loads" actually is.
