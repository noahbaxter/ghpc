...
# Stop thresholds. These live here rather than in a prompt because a stop
# condition counted in an agent's head does not survive a context compaction,
# and an unattended loop with a forgotten counter runs until something else
...
def _rendered_text(text, m):
    ...
    body.append("| rounds since gain | %d of %d |"
                % (m.get("rounds_since_gain", 0), STOP_ROUNDS_SINCE_GAIN))
    body.append("| rounds total | %d of %d |"
                % (m.get("rounds_total", 0), STOP_ROUNDS_TOTAL))
...
def round_done():
    """Count one completed round and say whether the loop should stop.

    Exit 3 means stop. It is a distinct code from the verdict codes on purpose:
    a caller must never confuse "this round did not advance" (0) with "stop
    running rounds" (3).
    """
    m = load_mark()
    if m is None:
        sys.exit("no mark recorded; run progress.py --record first")
    m["rounds_since_gain"] = m.get("rounds_since_gain", 0) + 1
    m["rounds_total"] = m.get("rounds_total", 0) + 1
    with open(MARK, "w") as f:
        json.dump(m, f, indent=2)
        f.write("\n")
    render()
    reasons = []
    if m["rounds_since_gain"] >= STOP_ROUNDS_SINCE_GAIN:
        reasons.append("%d rounds with no held-rung increase"
                       % m["rounds_since_gain"])
    if m["rounds_total"] >= STOP_ROUNDS_TOTAL:
        reasons.append("%d rounds this run" % m["rounds_total"])
    print("round %d counted, %d since the last gain"
          % (m["rounds_total"], m["rounds_since_gain"]))
    if reasons:
...
def main():
    ...
    ap.add_argument("--round-done", action="store_true",
                    help="count one completed round; exit 3 means stop looping")
        ...
        if run["bounced"]:
            ...
            with open(MARK, "w") as f:
                json.dump({"rung": rung, "screen": screen, "detail": detail,
                           ...
                           # A gain is what the "no progress" counter measures
                           # distance from, so recording one resets it. The
                           # total is carried, never reset: it is the hard cap.
                           "rounds_since_gain": 0,
                           "rounds_total": (mark or {}).get("rounds_total", 0),
